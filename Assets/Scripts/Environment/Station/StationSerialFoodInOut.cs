...
{
    public class StationSerialFoodInOut : MonoBehaviour
    {
        [Serializable]
        private class StationOutInfo
        {
            ...
            public UnityEvent OnFoodInSuccesful;
        ...
        [SerializeField, Foldout(MetaSetupFoldOut)] DataConsumable m_FoodTypeIn;
        ...
        private void OnTaskStart(TaskTarget taskTarget)
        {
            if (taskTarget.TryGetComponent(out IEnvironmentInteractable interactable))
            {
                ...
                {
                    if (interactable.GetDataConsumable(m_FoodTypeIn).Item1)
                    {
                ...
                foreach (var item in m_StationOutInfo)
                {
                    if (item.IsHolding)
                    {
                        if (interactable.SendDataConsumable(item.FoodOutType, m_CostProperty))
                        {
                            item.IsHolding = false;
                            item.OnFoodOutSuccesful?.Invoke();
                ...
                {
                    GlobalEventHolder.OnHintByConsumable?.Invoke(m_FoodTypeIn);
                }
            }
